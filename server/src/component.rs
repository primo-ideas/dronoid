...
#[derive(Component)]
pub(crate) struct Kind(pub(crate) dronoid_protocol::Kind);
...
impl Factory {
    pub(crate) fn spawn_dronoid(
        &mut self,
        rapier_bodies: &mut RigidBodySet,
        ...
        rapier_colliders.insert_with_parent(collider, rapier_hdl, rapier_bodies);
        commands.spawn((
            ZoneExtension { radius: 25. },
            RapierObject { rapier_hdl },
            Kind(dronoid_protocol::Kind::Dronoid),
        ));
    }
}
