...
use dronoid_protocol::AuthenticationRequest;
...
pub struct Connection {
    ...
    pub message_receiver: crossbeam_channel::Receiver<dronoid_protocol::ServerMessage>,
}
...
pub fn connect(
    ...
    match web_sys::WebSocket::new(addr.as_str()) {
        Ok(websocket) => {
            ...
            state.set(GameState::AuthenticateSendRequest);
        }
        Err(err) => {
            info_label.write(InfoMessage(
...
pub fn send_actions(connection: Res<Connection>, mut actions: MessageReader<ActionMessage>) {
    for action in actions.read() {
        let binary_to_send = bson::serialize_to_vec(&action.0).unwrap();
        connection
            .websocket
            .send_with_u8_array(&binary_to_send.as_slice())
            .unwrap();
    }
}
